...
actual fun executeCommand(vararg args: String): CommandExecutionResult =
    withCommandTempFile { stdout, stderr ->
        memScoped {
            ...
            val stdoutPath = stdout.toString()
            val stderrPath = stderr.toString()
            ...
            if (stdoutHandle == INVALID_HANDLE_VALUE) {
                return@memScoped CommandExecutionResult(
                    -1,
                    "",
                    "Failed to open stdout temp file (error=${GetLastError()})."
                )
            }
            ...
            if (stderrHandle == INVALID_HANDLE_VALUE) {
                ...
                return@memScoped CommandExecutionResult(
                    -1,
                    "",
                    "Failed to open stderr temp file (error=${GetLastError()})."
                )
            }
            ...
            try {
                ...
                if (
                    ...
                    return@memScoped CommandExecutionResult(
                        -1,
                        "",
                        "Failed to start process (error=${GetLastError()})."
                    )
                }
                ...
                CommandExecutionResult(
                    ...
                    stdout = withFs { stdout.readText() },
                    stderr = withFs { stderr.readText() },
                )
            } finally {
                if (stdoutHandleOpen) {
                    closeHandle(stdoutHandle)
