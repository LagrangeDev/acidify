...
此节介绍的是 Lagrange V2 签名 API 规范。在使用此种签名 API 时，请将配置文件中的 `protocol.pcLagrangeSignToken` 设为签名服务提供的 Token，并将 `protocol.uin` 设为实际登录的 QQ 号。`protocol.os` 必须为 `Windows`、`Mac` 或 `Linux`，`protocol.password` 可以留空。

此种签名 API 不支持使用 `protocol.version = "fetched"`，必须指定具体的协议版本号，或者使用 `custom` 并提供 `app-info.json` 文件。所用协议类型和版本需要与签名服务相匹配。
