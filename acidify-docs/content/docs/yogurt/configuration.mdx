...
在使用 `Windows`、`Mac` 或 `Linux` 协议时需要提供 `uin`，但 `password` 可以留空。
...
对接 Lagrange V2 风格的 Sign API 时使用的访问令牌，请咨询签名服务提供者获取。
...
### `protocol.autoRenewSession`

默认为 `true`，仅对 PC 协议生效。Yogurt 会在凭据获取或刷新满 15 天后使用 A1 自动续约并重新上线。旧版 SessionStore 未记录凭据获取时间，在上线后会随机等待 1～5 分钟进行首次续约。
